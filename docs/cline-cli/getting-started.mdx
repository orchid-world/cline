...
- **Keyboard shortcuts** - `Shift+Tab` to toggle Plan/Act, `Ctrl+Y` for auto-approve all
...
| Plan/Act toggle | ✓ (Shift+Tab) | `-p` / `-a` flags |
| Auto-approve | ✓ (Ctrl+Y) | `-y` flag |
| Session summary | ✓ | - |
| JSON output | - | `--json` |
| Piped input | - | ✓ |
...
- `Shift+Tab` to toggle between Plan and Act modes
- `Ctrl+Y` to enable auto-approve
